...
summary:
  pgmem:
    ...
    weaknesses: [one session per fork with transaction-mode multiplexing (rule:single-session-per-backend), plpgsql only until requirement:cloud-common-extensions, ~150 MB resident per process, ~36 MB binary]
  testcontainers_docker:
    shape: Docker daemon runs the official postgres image; the test framework starts and stops containers
    strengths: [exact production image and version, all extensions, real multi-session server, mature ecosystem in every language]
  ...
  pglite:
    ...
    strengths: [in-process for JavaScript, extensions including pgvector, browser support]
  ...
  choose_docker_when: [tests need extensions pgmem does not link, tests exercise concurrency between sessions, an exact server build must be pinned]
```
