...
system:
  prisma_7_10:
    ...
    migrate_dev: works since concept:database-switching (2026-09-13); Prisma creates prisma_migrate_shadow_db_<uuid> on the same server, uses and drops it; verified init 1.3s, a second migration 0.6s, an already-in-sync run, migrate status, db pull; before that it failed with P1003
  ...
  hazard_outer_client_in_transaction: db.transaction(async tx => { await db.select() }) works on real PostgreSQL but cannot proceed on pgmem; it waited forever until Options.WaitTimeout, and now ends after 2s with 55P03 naming both connections (rule:single-session-per-backend)
  env_loading: dotenv and NestJS ConfigModule do not override variables that are already set
  with_reset_2026_09_13: through @pgmem/core, Prisma's pooled client, drizzle named .prepare and postgres.js keep working across api:reset; the outer-client-in-transaction case ends after 2s with 55P03, shown by Prisma as 'Database error. Code: 55P03' with pgmem's message; a fork closed under drizzle's idle pool no longer crashes the worker
```
