...
# jp.shibu:pgmem and pgmem-junit5 (jar, sources, javadoc) and pgmem-native
...
for classifier in darwin-arm64 darwin-x86_64 linux-arm64 linux-x86_64 windows-arm64 windows-x86_64; do
  jar=$staging/jp/shibu/pgmem-native/$version/pgmem-native-$version-$classifier.jar
  if [ ! -f "$jar" ]; then echo "missing $jar"; status=1; fi
done
[ $status = 0 ]

mkdir -p dist/maven
(cd "$staging" && zip -qr "$root/dist/maven/pgmem-$version-bundle.zip" jp -x '*maven-metadata.xml*')
