...
L29:
	;
	m.Env.X__assert_fail(m, int32(578871), int32(518584), int32(150), int32(582759))
	mBase = m.M
...
L30:
	;
	m.Env.X__assert_fail(m, int32(582624), int32(518584), int32(121), int32(582759))
	mBase = m.M
	base.Wasm_trap_unreachable()
	for {
	}
L31:
	;
	m.Env.X__assert_fail(m, int32(578871), int32(518584), int32(301), int32(582740))
	mBase = m.M
	base.Wasm_trap_unreachable()
	for {
	}
L32:
	;
	m.Env.X__assert_fail(m, int32(578871), int32(518584), int32(276), int32(582740))
	mBase = m.M
	base.Wasm_trap_unreachable()
	for {
	}
...
L237:
	;
	m.Env.X__assert_fail(m, int32(578871), int32(518584), int32(181), int32(582759))
	mBase = m.M
	base.Wasm_trap_unreachable()
	for {
	}
...
L318:
	;
	F_errsave_finish(m, l2, int32(520421), int32(100), int32(291911))
	mBase = m.M
	v1372 = m.ExcPending
	if v1372 != 0 {
		goto L1
...
L339:
	...
	*(*int32)(unsafe.Add(mBase, uint32(v21)+16)) = l0
	F_errmsg(m, int32(758601), v21+int32(16))
	mBase = m.M
	v1452 = m.ExcPending
	if v1452 != 0 {
		goto L1
...
L340:
	;
	F_errdetail(m, int32(649822), int32(0))
	mBase = m.M
	v1456 = m.ExcPending
	if v1456 != 0 {
		goto L1
...
L341:
	;
	F_errsave_finish(m, l2, int32(520421), int32(111), int32(291911))
	mBase = m.M
	v1461 = m.ExcPending
	if v1461 != 0 {
		goto L1
			...
			if v38 != 0 {
				...
				if v49 == int32(0) {
					...
					*(*int32)(unsafe.Add(mBase, uint32(v9))) = v59
					v67 = F_pg_snprintf(m, v54+(v9+int32(16)), int32(50)-v54, int32(41734), v9)
					mBase = m.M
					v68 = m.ExcPending
					if v68 != 0 {
						return int32(0)
				...
				if v81 != 0 {
					...
					if v84 != 0 {
						...
						F_errmsg(m, int32(307372), int32(0))
						mBase = m.M
						v88 = m.ExcPending
						if v88 != 0 {
							return int32(0)
						} else {
							F_errfinish(m, int32(520421), int32(1174), int32(33060))
							mBase = m.M
							v93 = m.ExcPending
							if v93 != 0 {
								return int32(0)
