...
func F_ResOwnerPrintRelCache(m *base.Module, l0 int32) int32 {
	...
	*(*int32)(unsafe.Add(mBase, uint32(v5))) = v7 + int32(4)
	v12 = F_psprintf(m, int32(741625), v5)
	mBase = m.M
	v15 = m.ExcPending
	if v15 != 0 {
		return int32(0)
	...
	if l0 != 0 {
		...
		if v58 != 0 {
			...
			*(*int32)(unsafe.Add(mBase, uint32(v6))) = int32(0)
			F_errmsg_internal(m, int32(509456), v6)
			mBase = m.M
			v63 = m.ExcPending
			if v63 != 0 {
				return
			} else {
				F_errfinish(m, int32(518217), int32(6561), int32(292945))
				mBase = m.M
				v68 = m.ExcPending
				if v68 != 0 {
					return
	...
	*(*int32)(unsafe.Add(mBase, uint32(l0)+96)) = v2
	...
	if v5 <= v2 {
		...
		if v9 == int32(0) {
			...
			F_ResourceOwnerForget(m, v9, l0, int32(4432748))
			mBase = m.M
			v14 = m.ExcPending
			if v14 != 0 {
				return
