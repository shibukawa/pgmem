...
<Aside>
A fork is a PostgreSQL cluster of its own, so a connection pool on one fork works as on a server. Commit or close connections to the template before `snapshot()`, or it fails with `busy` after 30 seconds.
</Aside>
