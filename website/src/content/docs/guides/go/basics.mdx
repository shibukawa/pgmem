...
A snapshot checkpoints the server and clones its data directory. A fork clones the snapshot and starts a new backend; the clones share file contents until a file is written, so a fork costs milliseconds and the memory of the files it changes. Commit or close every connection to `s` before `Snapshot`, because it waits for open transactions.
