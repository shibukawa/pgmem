---
title: pgmem
description: テストの中でメモリ上に動く本物の PostgreSQL 18。Docker 不要、ディスク不要、テストごとの複製はミリ秒単位。
template: splash
hero:
  tagline: テストのための、メモリ上で動く本物の PostgreSQL 18。Docker もディスクも使いません。スキーマは一度だけ準備し、各テストにはその複製をミリ秒で渡します。
  actions:
    - text: クイックスタート
      link: quickstart/
      variant: minimal
...
クエリ性能、計測方法、再現手順は[ベンチマーク](benchmarks/)のページにあります。
...
<CardGrid>
	...
	<Card title="テストごとにフォーク" icon="random">
		マイグレーションとシードは一度だけ。スナップショットを取り、各テストには独立した複製を渡します。書き込むテスト同士も並列に走らせられます。
	</Card>
