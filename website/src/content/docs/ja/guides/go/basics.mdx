...
[Go のテストガイド](../testing/)では、データベースを一度準備し、読み取り専用テストではフォークを共有し、書き込みテストだけに専用の複製を渡す方法を説明しています。小さなクエリではプロセス内ダイヤラが TCP 接続より約 3 倍速くなります。[ベンチマーク結果](/pgmem/ja/benchmarks/)を参照してください。
...
1. **サーバーを起動します。** `pgmem.Start` は、専用のメモリ上データディレクトリを持つ PostgreSQL を起動して返します。
   ...
   `Options` にはほかに `User`、`Port`（0 なら空きポート）、`postgres -c` の設定を渡す `Params`、サーバーログを受け取る `Log`、ほかの接続のアイドル状態のトランザクションの後ろで待てる時間を決める `WaitTimeout` があります。
...
`Server.Dial` はループバックソケットではなく `net.Pipe` で接続します。TCP の往復は時間の大半を PostgreSQL ではなくカーネルで使うので、小さなクエリならおよそ 3 倍速くなります。`pgmemtest.Fixture.PgxPool` と `pgmemtest.Fixture.DB` はこのダイヤラを設定してくれます。
...
<Aside>
サーバーは持っているすべてのデータベースを提供し、接続が別のデータベースに切り替わるとバックエンドを再起動します。同じサーバーに作業用のデータベースを作るツールも使えますが、切り替えのたびに再起動のコストがかかるので、テストケースではなく準備の段階にとどめてください。
</Aside>
...
[dbtestify](https://github.com/shibukawa/dbtestify) は YAML のデータセットをシードとして投入できます。バックエンドのユニットテストでは、実行後のテーブル内容を期待するデータセットと比較し、差分を確認する用途にも使えます。
...
スナップショットはサーバーをチェックポイントしてデータディレクトリをクローンします。フォークはスナップショットをクローンして新しいバックエンドを起動します。クローン同士はファイルの中身を共有し、書き込んだファイルだけが複製されるので、フォークのコストは数ミリ秒と、変更したファイルぶんのメモリだけです。`Snapshot` は開いているトランザクションを待つので、その前に `s` への接続はすべてコミットするか閉じてください。
