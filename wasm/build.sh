...
# pgmem: build-path independence
# The compiled module is committed as Go source, so the directory it was built
# in must not reach its static data. See buildpaths.py.
emmake make PORTNAME=emscripten -C src/port pg_config_paths.h >/dev/null 2>&1 || true
python3 "$HERE/buildpaths.py" "$SRC" "$ROOT" "$ROOT/toolchain/emsdk"
...
CONTRIB_MODULES="pgcrypto citext pg_trgm hstore ltree btree_gist btree_gin unaccent tablefunc intarray fuzzystrmatch cube earthdistance seg bloom isn dict_int dict_xsyn lo tsm_system_rows tsm_system_time pgstattuple uuid-ossp amcheck pg_visibility pageinspect pg_buffercache pg_freespacemap pg_prewarm"
