...
// A connection that waits behind another connection's idle transaction is
...
func TestWaitBehindIdleTransactionFails(t *testing.T) {
	...
	waiter, err := pgx.Connect(ctx, s.DSN())
	if err != nil {
		t.Fatal(err)
	}
	defer waiter.Close(ctx)
	if _, err := holder.Exec(ctx, `CREATE TABLE t(v int); BEGIN; INSERT INTO t VALUES (1)`); err != nil {
		t.Fatal(err)
	}
	start := time.Now()
	_, err = waiter.Exec(ctx, `SELECT count(*) FROM t`)
	var pgErr *pgconn.PgError
	if !errors.As(err, &pgErr) || pgErr.Code != "55P03" || !strings.Contains(pgErr.Message, "idle inside a transaction") {
		t.Fatalf("waiter: %v, want 55P03", err)
	}
	if d := time.Since(start); d > 3*time.Second {
		t.Fatalf("waiter failed after %s", d)
	}
	t.Log(pgErr.Message)
	if _, err := holder.Exec(ctx, `COMMIT`); err != nil {
		t.Fatal(err)
	}
	if n := count(t, s.DSN()); n != 1 {
...
func TestWaitBehindRunningStatementSucceeds(t *testing.T) {
	...
	defer slow.Close(ctx)
	other, err := pgx.Connect(ctx, s.DSN())
	if err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		_, err := slow.Exec(ctx, `SELECT pg_sleep(1)`)
		done <- err
	}()
	time.Sleep(100 * time.Millisecond)
	start := time.Now()
	if _, err := other.Exec(ctx, `SELECT 1`); err != nil {
		t.Fatalf("waiting behind a running statement: %v", err)
	}
	if d := time.Since(start); d < 500*time.Millisecond {
		t.Fatalf("second statement did not wait for the first (%s)", d)
	}
	if err := <-done; err != nil {
		t.Fatal(err)
	}
